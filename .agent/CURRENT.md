...
- **Stage 16W: ACTIVE / PARTIAL — USER APPROVED 2026-09-12.** Preview 1 remains published. The newer functional build, tests, portable packaging, checksum, and artifact upload passed in Windows Server 2022 run `34761647634`. A separately verified `v0.1.0-windows-preview.2` prerelease draft with both assets is ready but not public; publishing awaits immediate user confirmation. Feedback-form run `34799244166` also passed Windows build, tests, bundling, and upload. Real Windows-machine acceptance and the deferred daily-use features remain mandatory before `PASS`.
...
- Branch `codex/windows-port`; latest branch commit `709cc90` adds a branch-independent, privacy-safe Windows feedback link; accepted macOS product baseline on `main`: `776b2ef` (`Remove Outlook integration and finalize dashboard updates`). Functional Windows source commit `4c9aab6` passed run `34761647634`; release-draft workflow run `34779276068` selected its exact artifact by run and artifact id, verified provenance, checksum and archive contents, and created GitHub draft release id `388016887`. Feedback-form commit `5e4c4ba` independently passed Windows run `34799244166`.
...
1. Obtain immediate confirmation and publish the verified `v0.1.0-windows-preview.2` draft without changing its tested assets.
2. Ask the user or their friend to complete the real Windows launch, restart, credential, Canvas read-only sync, navigation, scaling, keyboard, localization, foreground-reminder, offline, clear-data, and privacy smoke matrix.
3. Return defects to Stage 16W and continue the remaining SIweb, DeepSeek, full persistence, native/background reminders, signed installer, and physical-machine accessibility work before daily-use acceptance.
