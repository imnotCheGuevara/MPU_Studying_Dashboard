...
- **macOS 0.3.1 (5) published — 2026-09-17:** User authorized publishing the current repaired Mac application and replacing the old download entry. Candidate `dist/macos-0.3.1/Campus Dashboard.app`; 250 tests, Release build, strict signature and packaged Keychain smoke passed. Apple Silicon / macOS 14+. DeepSeek model is `deepseek-flash`; renewed in-app consent and per-announcement reprocessing are required. Live provider verification on the friend's machine remains pending. Published as latest GitHub release `v0.3.1-macos`, source `8a4d44a`; remote asset digest and downloaded ZIP verified. Default main README now links the new download. See `.agent/handoffs/macos-release-2026-09-17.md`.
...
- Branch `codex/windows-port`; Windows Preview 3 functional source commit `d43707a76c9125587e428004deb2e3110b7b99b0` passed run `34806098324`. The selected artifact id is `10334710441`, with Actions wrapper digest `sha256:61efd4dcd3b601df6acdbb6d9a748829f8ae7c9fcb27fd1ff09cf6e4dd4ae41f`; its inner portable ZIP digest is `sha256:d18b32e760a19acd4d81e0d07abfd884c3a15736a5e90b0db637c94ac99b2c7d`. Documentation and feedback commits through `03da86a` add the Simplified Chinese friend-testing guide, a Preview 3-aware privacy-safe feedback form, and a clickable pre-filled report entry; the draft targets that commit while preserving binary provenance at `d43707a`. Accepted macOS product baseline on `main` remains `776b2ef` (`Remove Outlook integration and finalize dashboard updates`).
