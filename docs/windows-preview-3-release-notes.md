...
Download both assets and verify the ZIP against `SHA256SUMS.txt` before extracting it. If Windows App Runtime is missing, run the bundled `WindowsAppRuntimeInstaller.exe` once. Full setup, SIweb session, upgrade, uninstall, privacy, and smoke-test instructions are in [the Windows preview testing guide](https://github.com/imnotCheGuevara/MPU_Studying_Dashboard/blob/codex/windows-port/docs/windows-preview-testing.md).
